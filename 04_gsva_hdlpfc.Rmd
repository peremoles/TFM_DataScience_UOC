...
#system.time(spe.nnsvg <- nnSVG(spe.filt[top_hvgs,], assay_name = "logcounts", BPPARAM=MulticoreParam(workers=4)))
...
```{r}
#rowData(spe.nnsvg)[order(rowData(spe.nnsvg)$rank)[1:10], ]
```
...
for(j in 1:4){
  ...
  gs <- gs[1:min(8,length(gs))]
  
 # df <- sapply(gs, function(g){
  #  stat <- rowData(spe.nnsvg)[rowData(spe.nnsvg)$gene_id == g, ]
   # stat
  #})
  #df <- do.call(rbind, df)
  #print(head(df))
