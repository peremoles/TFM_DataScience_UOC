---
title: "GSVA for SpatialExperiment class: Human Heart analysis"
output: 
  html_document:
    css: bootstrap.css
    toc: true
...
qc_mito <- colData(spe)$subsets_mito_percent > 50
```
...
```{r}
saveRDS(spe, file="RDS_files/spe.rds")
```
...
```{r}
cellmarkers <- read.xlsx('Cell_marker_Human.xlsx', rowNames =F)
```
Extract Gene Set info from the xlsx file
```{r}
cell_types <- cellmarkers %>% filter(tissue_class=='Heart' & cell_type=="Normal cell") %>% dplyr::count(cell_name) %>% arrange(desc(n)) %>% filter(n>1) %>% select(cell_name)
cell_types <- cell_types$cell_name


# filter genes symbol info on the xlsx file
cm_sets <- lapply(cell_types, function(x) cellmarkers %>% 
                    filter(tissue_class=='Heart' & 
...
colGraph(gsva_sfe, "visium") <- findVisiumGraph(gsva_sfe, zero.policy = TRUE)
...
```{r}
rowData(moran_res)[order(rowData(moran_res)$moran_HumanHeart,decreasing = TRUE),][1:10,2:3]
```
### Plot Top 10 Spatially Autocorrelated Genesets

```{r}

top_SVG_genesets <- rownames(rowData(moran_res)[order(rowData(moran_res)$K_HumanHeart,decreasing = TRUE),][1:10,])
